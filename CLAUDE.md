...
- `BBP_PAIRINGS_BIN` - (Опционально) Путь к бинарнику BBP Pairings для генерации пар по правилам ФИДЕ
...
- Предпочтительный движок для генерации пар по правилам ФИДЕ
- Запускается как нативный бинарь (требует `BBP_PAIRINGS_BIN` или наличие в PATH)
- Генерирует TRF(bx) файл, вызывает бинарь, парсит выходные данные
- При ошибке или отсутствии бинарника — пары НЕ генерируются, UI показывает 502-специфичное сообщение; fallback отсутствует (BBP-only mode)
